...
let github, codeIntel, finance, notionSync, analyticsPortfolio;
...
try { analyticsPortfolio = require("./collectors/analytics-portfolio"); } catch (e) {
  log.warn({ err: e.message }, "Analytics Portfolio collector not available");
}
let auth0;
try { auth0 = require("./collectors/auth0"); } catch (e) {
  log.warn({ err: e.message }, "Auth0 collector not available");
}
let infisical;
try { infisical = require("./collectors/infisical"); } catch (e) {
  log.warn({ err: e.message }, "Infisical collector not available");
}
...
const collectors = [
  ...
  { name: "analytics-portfolio", module: analyticsPortfolio, enabled: false, timer: null },
  { name: "auth0",               module: auth0,              enabled: false, timer: null },
  { name: "infisical",           module: infisical,          enabled: false, timer: null },
];
...
function getDashboard() {
  ...
  if (analyticsPortfolio?.getSummary) {
    modules.analyticsPortfolio = analyticsPortfolio.getSummary();
  }
  if (infisical?.getSummary) {
    modules.infisical = infisical.getSummary();
  }
  ...
  // Portfolio analytics health (avg activation rate as %)
  if (modules.analyticsPortfolio?.avgActivationRate != null) {
    const activationHealth = Math.min(100, Math.round(modules.analyticsPortfolio.avgActivationRate * 100));
    healthFactors.push({ name: "Portfolio Activation", score: activationHealth, weight: 2 });
  }

  if (modules.infisical?.summary?.total > 0) {
    const syncHealth = Math.round((modules.infisical.summary.synced / modules.infisical.summary.total) * 100);
    healthFactors.push({ name: "Secrets Sync", score: syncHealth, weight: 2 });
  }
...
function getProjectView(projectName) {
  ...
  if (analyticsPortfolio?.getProjectDetail) {
    view.analytics = analyticsPortfolio.getProjectDetail(projectName);
  }
